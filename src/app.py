...
def run_app():
    ...
    This tool can combinee data from any two lists, such as inventories, customer records, or project data. 
    Choose a merge type, including exact matches (left, inner, outer, right, anti), fuzzy matches 
    (fuzzy, fuzzy anti), or a cross merge for all combinations. 
    """)
    ...
    if st.session_state.df1 is not None and st.session_state.df2 is not None:
        ...
        # Fuzzy threshold for fuzzy joins
        if merge_type in ["fuzzy", "fuzzy anti"]:
            fuzzy_threshold = st.slider("Fuzzy Join Threshold (0-100)", min_value=50.0, max_value=100.0, value=90.0, step=0.1, key="fuzzy_threshold")
        else:
        ...
        # Perform join when button is clicked
        if st.button("Merge Files", key="merge_button"):
            ...
            try:
                ...
                st.download_button(
                    ...
                    data=result,
                    file_name=f"merged_output{'.csv' if file1.name.endswith('.csv') else '.xlsx'}",
